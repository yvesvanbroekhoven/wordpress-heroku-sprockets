 ...
 *
 * The error-checking in this plugin isn't great.
 *
 * Need to check that Cloudf
 *
 */
...
//		}
	...
	function addSubMenu()
	{
		add_submenu_page('aws_for_wp', 'Amazon S3', 'Amazon S3', 'manage_options', 'aws_for_wp_amazon_s3', array($this, 'admin'));
		$this->version_check();
	}

	/**
	 * Check to see whether our S3 upload is currently enabled.
	 */
	function isEnabled()
	{
		$options = get_option('tantan_wordpress_s3');

		return (isset($options['wp-uploads']) && $options['bucket'] != '');
	}
    ...
    function admin() {
        if ( isset( $_POST['action'] ) && $_POST['action'] == 'save' ) {
	        ...
	        // $_POST['options']['wp-uploads'] = false;
            ...
            if (isset($_POST['options']['bucket']) && $_POST['options']['bucket']) {
                ...
                if (!in_array($_POST['options']['bucket'], $s3->listBuckets())) {
                    if ($s3->createBucket($_POST['options']['bucket'],'public-read')) {
	                    ...
	                    // Error: set the bucket to blank and disable the plugin
	                    $options['bucket'] = '';
						if (isset($options['wp-uploads'])) {
	                        unset($options['wp-uploads']);
						}
	                    update_option('tantan_wordpress_s3', $options);

                        $error = "The bucket <strong>".$_POST['options']['bucket'].'</strong> is not available. Please choose a different name.';
                    }
                } else {
	    ...
	    // Check to see that we have a bucket.
	    if (isset($options['bucket'])) {
		    if ($options['bucket'] == '' && $options['cloudfront'] == '') {
			    if (isset($options['wp-uploads'])) {
				    unset($options['wp-uploads']);
			    }
			    update_option('tantan_wordpress_s3', $options);
    ...
    */
	...
	function media_buttons($context) {
		...
		// Don't show a media button unless S3 has been configured
