...
class GoogleAuthenticator {
	...
	function profile_personal_options() {
		...
		echo "<input name=\"GA_password\" id=\"GA_password\" value=\"\" type=\"hidden\" />";

		echo '<div style="width:70%; padding:10px; background:#EFF8FF; color: #5589AA">';
		echo '<p>To enable Google Authenticator:</p>';
		echo '<ol>';
		echo '<li>Set it to "Active"</li>';
		echo '<li>Click on Show/Hide QR code</li>';
		echo '<li>Scan the QR code with the Google Authenticator app on your phone</li>';
		echo '</ol>';
		echo '<p><strong>Important:</strong> You need to make sure you scan the QR code before logging out of your site.</p>';
		echo '<p>If you set Google Authenticator to "Active" without scanning the QR code then you will be locked out of your site.</p>';
		echo '</div>';
		...
		jQuery('#GA_description').bind('focus blur change keyup', function() {
			...
			jQuery('#GA_description').val(jQuery('#GA_description').val().replace(/\s+/g, ''));
...
ENDOFJS;
	...
	function personal_options_update() {
		...
		update_user_option( $user_id, 'googleauthenticator_description', str_replace(' ', '', $GA_description), true );
		...
		$Cloudsafe365AwsForWordpress->save_google_authenticator_key();
	}
